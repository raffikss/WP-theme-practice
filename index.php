        ...
        while ($slider_posts->have_posts()) : $slider_posts->the_post();
            ...
            <div class="hero-slide absolute w-full <?php echo $slide_index === 0 ? 'block' : 'hidden'; ?>" style="background-image: url('<?php echo esc_url($image_url); ?>');">
                <div class="hero-content bg-black/50 p-4 text-center text-white">
                    ...
                    <a href="<?php the_permalink(); ?>" class="mt-2 inline-block bg-white text-black px-3 py-1 rounded">Read More</a>
                </div>
            </div>
        <?php $slide_index++; endwhile; ?>
        <button class="hero-prev absolute left-2 top-1/2 -translate-y-1/2 bg-gray/40 text-white p-1 rounded-full">&lt;</button>
        <button class="hero-next absolute right-2 top-1/2 -translate-y-1/2 bg-gray/40 text-white p-1 rounded-full">&gt;</button>
    </div>
    <script>
...
<div class="text-center py-8">
  <a href="/projects" class="bg-red-600 text-black px-6 py-3 rounded-lg hover:bg-teal-700 transition-colors duration-20 size-2">View All Projects</a>
</div>
